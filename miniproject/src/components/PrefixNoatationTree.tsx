...
const BSTPrefixVisualization: React.FC = () => {
  const [inputValue, setInputValue] = useState<string>("");
  const [operation, setOperation] = useState<string>("");
  const [prefixNotation, setPrefixNotation] = useState<number[]>([]);
  const [isAnimatingPrefix, setIsAnimatingPrefix] = useState<boolean>(false);
  const [bst] = useState<BST>(new BST());
  const svgRef = useRef<SVGSVGElement>(null);
  const gRef = useRef<SVGGElement>(null);
  const animationRef = useRef<any>(null);
  const prefixAnimationRef = useRef<any>(null);
  ...
  useEffect(() => {
    const initialValues = [47, 23, 89, 12, 56, 34, 76, 8, 61, 92, 41];
    initialValues.forEach((val) => bst.insert(val));
    updateTree();

    return () => {
      if (animationRef.current) clearInterval(animationRef.current);
      if (prefixAnimationRef.current) clearInterval(prefixAnimationRef.current);
    };
  }, []);
  ...
  // Collect nodes in prefix order
  const getPrefixOrder = (): TreeNode[] => {
  ...
  // Animate prefix traversal
  const animatePrefixTraversal = () => {
    if (!bst.root) return;

    setIsAnimatingPrefix(true);
    setPrefixNotation([]);
    const g = d3.select(gRef.current);
    const nodes = getPrefixOrder();

    if (nodes.length === 0) {
      setIsAnimatingPrefix(false);
      return;
    }

    if (prefixAnimationRef.current) {
      clearInterval(prefixAnimationRef.current);
    }

    g.selectAll(".node-circle").classed("prefix", false);
    ...
    const animateNextNode = () => {
      ...
      setPrefixNotation((prev) => [...prev, currentNode.value]);

      i++;
    };

    animateNextNode();
    prefixAnimationRef.current = setInterval(animateNextNode, 1000);
  };
    ...
    <div className="bst-visualization">
      <div className="controls">
        ...
        <button
          ...
          disabled={isAnimatingPrefix}
        >
          {isAnimatingPrefix ? "Traversing..." : "Prefix Notation"}
        </button>
      ...
      <div className="prefix-display">
        ...
        <div className="prefix-values">
          {prefixNotation.map((value, index) => (
            <span
              key={index}
              className={`prefix-value ${
                index === prefixNotation.length - 1 ? "current" : ""
              }`}
            >
              {value}
              {index < prefixNotation.length - 1 ? ", " : ""}
            </span>
          ))}
        </div>
      </div>
      ...
      <style>{`
        ...
        .button.prefix:hover:not(:disabled) {
...
export default BSTPrefixVisualization;
